...
def get_planning_constraints(llm_response, llm_plan_so_far):
    ...
    valid_actions = ['(get_wood1)', '(get_wood0)', '(get_processed_wood)', '(make_stick)', '(make_plank)', '(make_ladder)']
    ...
    if llm_response not in valid_actions:
        ...
        if '(get_wood1)' in llm_response or '(get_wood0)' in llm_response:
            FEASIBLE = True
                
        elif '(get_processed_wood)' in llm_response:
            if '(get_wood1)' in llm_plan_so_far and '(get_wood0)' in llm_plan_so_far:
                FEASIBLE = True
            else:
                FEASIBLE = False
        ...
        elif '(make_stick)' in llm_response or '(make_plank)' in llm_response:
            if '(get_processed_wood)' in llm_plan_so_far:
                FEASIBLE = True
            else:
                FEASIBLE = False
                backprompt = 'Your plan so far: ' + str(llm_plan_so_far) + '\n\n' + 'Your response: ' + llm_response + '\n\n' + 'The action provided is not feasible. Please choose a valid action from the list ' + str(valid_actions) + '\n.'
            ...
            if '(make_stick)' in llm_plan_so_far and '(make_plank)' in llm_plan_so_far:
                FEASIBLE = True
            else:
                FEASIBLE = False
                backprompt = 'Your plan so far: ' + str(llm_plan_so_far) + '\n\n' + 'Your response: ' + llm_response + '\n\n' + 'The action provided is not feasible. Please choose a valid action from the list ' + str(valid_actions) + '\n.'
