...
class DoorKey6x6Prompts(EnvPrompts):
    ...
    def _convert_pos_to_4x4(self, pos):
        return (pos[0]-1, pos[1]-1)
    ...
    def convert_obs_to_text(self, observation):
        ...
        for i in range(1,5):
            for j in range(1,5):
                if obs_array[i][j][2] == OBJECT_TO_IDX['agent']:
                    x, y = obs_array[i][j][0], obs_array[i][j][1]
                    agent_pos = self._convert_pos_to_4x4((x, y))
                if obs_array[i][j][2] == OBJECT_TO_IDX['door']:
                    x, y = obs_array[i][j][0], obs_array[i][j][1]
                    door_pos = self._convert_pos_to_4x4((x, y))
                if obs_array[i][j][2] == OBJECT_TO_IDX['key']:
                    x, y = obs_array[i][j][0], obs_array[i][j][1]
                    key_pos = self._convert_pos_to_4x4((x, y))
                if obs_array[i][j][2] == OBJECT_TO_IDX['goal']:
                    x, y = obs_array[i][j][0], obs_array[i][j][1]
                    goal_pos = self._convert_pos_to_4x4((x, y))
                if obs_array[i][j][2] == OBJECT_TO_IDX['wall']:
                    x, y = obs_array[i][j][0], obs_array[i][j][1]
                    walls.append(self._convert_pos_to_4x4((x, y)))
                if obs_array[i][j][2] == OBJECT_TO_IDX['unseen']:
                    x, y = obs_array[i][j][0], obs_array[i][j][1]
                    unseen.append(self._convert_pos_to_4x4((x, y)))
        
        AGENT_DESC = f"In the 4x4 grid with (0, 0) at top-left and (2,2) at bottom right, you are at position {agent_pos}."
        KEY_DESC = f"The key is at position {key_pos}."
        DOOR_DESC = f"The door is at position {door_pos}."
        GOAL_DESC = f"The goal is at position {goal_pos}."
        MISC_DESC = f"There are walls at positions {walls}. There are unseen areas at positions {unseen}."
    ...
    def convert_obs_to_grid_text(self, observation):
        ...
        for i in range(4):
            # append key from OBJECT_TO_IDX    
            col0.append(obs_array[1][1:5][i][2])
            col1.append(obs_array[2][1:5][i][2])
            col2.append(obs_array[3][1:5][i][2])
            col3.append(obs_array[4][1:5][i][2])
            
        # transpose numpy array
        obs_matrix = np.array([col0, col1, col2, col3])
        obs_matrix = np.transpose(obs_matrix)
        row0, row1, row2, row3 = list(obs_matrix[0]), list(obs_matrix[1]), list(obs_matrix[2]), list(obs_matrix[3])
        ...
        if 'key' not in row0 and 'key' not in row1 and 'key' not in row2 and 'key' not in row3:
            ADDITIONAL_INFO = "You have already picked up the key.\n"
            AGENT_DIR += ADDITIONAL_INFO
